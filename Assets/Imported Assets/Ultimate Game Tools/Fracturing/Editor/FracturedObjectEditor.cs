...
public class FracturedObjectEditor : Editor
{
    ...
    SerializedProperty PropChunkColliderType;
    ...
    SerializedProperty PropUseSettings;
    SerializedProperty PropBuildingValue;
    SerializedProperty PropMinExplosionThreshold;
    SerializedProperty PropChunkVapourisationForce;
    SerializedProperty PropEmissionFinal;
    SerializedProperty PropEmissionLerp;
    ...
    void OnEnable()
    {
        PropBuildingValue = serializedObject.FindProperty("buildingValue");
        PropMinExplosionThreshold = serializedObject.FindProperty("explosionForceThreshold");
        PropChunkVapourisationForce = serializedObject.FindProperty("chunkVapourisationForce");
        PropUseSettings = serializedObject.FindProperty("initializeSettingsForMegaton");
        PropEmissionFinal = serializedObject.FindProperty("finalEmissionColor");
        PropEmissionLerp = serializedObject.FindProperty("emissionLerpTime");
        ...
        PropChunkColliderType                       = serializedObject.FindProperty("ChunkColliderType");
        ...
        if(fracturedComponent.ShowChunkConnectionLines)
        {
            ...
            foreach(FracturedChunk chunkA in fracturedComponent.ListFracturedChunks)
            {
                if(chunkA)
                {
                    ...
                    {
                        Handles.DotHandleCap(0, chunkA.transform.position, Quaternion.identity, HandleUtility.GetHandleSize(chunkA.transform.position) * 0.05f, Event.current.type);
                    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(fracturedComponent.GUIExpandMain)
        {
            ...
            EditorGUILayout.PropertyField(PropChunkColliderType, new GUIContent("Chunk Collider Type", "Use Collider if you want normal behaviour. Use Trigger only if you want to prevent a fast moving object from losing momentum when impacting a breakable object, for example a cannon ball hitting a wall. In this case both methods will break the wall but using Collider will make the cannon ball lose a lot of its speed, which may not be desired sometimes. On the other hand, if an object doesn't have enough mass/velocity to break a breakable object, using Trigger will make it go simply go through unrealistically. Using Collider it will bounce off correctly."));
        }
        ...
        if(fracturedComponent.GUIExpandSupportPlanes)
        {
            ...
            if(GUILayout.Button(new GUIContent("Add Support Plane", "Adds a new support plane to this object. Support planes control which chunks are tagged as support. Chunks that act as support can't be destroyed and will hold the object together. A chunk needs to be connected to a support chunk (directly or through other chunks) or otherwise it will fall. This prevents chunks from staying static in the air and enables realistic collapsing behavior."), GUILayout.Width(200)))
            {
                ...
                if(PropSaveMeshDataToAsset.boolValue && fracturedComponent.MeshAssetDataFile != null && fracturedComponent.MeshAssetDataFile.Length > 0)
                {
                    if(System.IO.File.Exists(fracturedComponent.MeshAssetDataFile))
                    {
                        UnityEditor.AssetDatabase.AddObjectToAsset(fracturedComponent.ListSupportPlanes[fracturedComponent.ListSupportPlanes.Count - 1].planeMesh, fracturedComponent.MeshAssetDataFile);
                    }
                    else
                    {
                        UnityEditor.AssetDatabase.CreateAsset(fracturedComponent.ListSupportPlanes[fracturedComponent.ListSupportPlanes.Count - 1].planeMesh, fracturedComponent.MeshAssetDataFile);
                    }
                }
            }
        ...
        PropUseSettings.boolValue = EditorGUILayout.Toggle(new GUIContent("Initialize Megaton Settings:", "If this is checked, it will initialize components used in Megaton"), PropUseSettings.boolValue);
        PropBuildingValue.floatValue = EditorGUILayout.FloatField(new GUIContent("Building Value: ", "Total Value of the building. This will be used to assign value to all of the chunks."), PropBuildingValue.floatValue);
        PropMinExplosionThreshold.floatValue = EditorGUILayout.FloatField(new GUIContent("Minimim Force Threshold:", "Muinimum force needed to detach a chunk from the object."), PropMinExplosionThreshold.floatValue);
        PropChunkVapourisationForce.floatValue = EditorGUILayout.FloatField(new GUIContent("Vapourisation force: ", "Force needed to vapourise a fractured chunk"), PropChunkVapourisationForce.floatValue);
        PropEmissionFinal.colorValue = EditorGUILayout.ColorField(new GUIContent("Final Emission Colour:", "This will set the final Emission Color of the interior Material"), PropEmissionFinal.colorValue);
        PropEmissionLerp.floatValue = EditorGUILayout.FloatField(new GUIContent("Emissive Lerp Time: ", "Amount of time it will take the interior material to lerp to the final color"), PropEmissionLerp.floatValue);
        ...
        if (GUILayout.Button(new GUIContent("Recompute Chunk Values", "Recomputes the Megaton values for all of the chunks, without recalculating the cells and colliders")))
        {
            fracturedComponent.UpdateChunkValues();
        }

        EditorGUILayout.Space();

#if USE_VHACD
        PropIntegrateWithConcaveCollider.boolValue = EditorGUILayout.Toggle(new GUIContent("Use Concave Collider", "Use the VHACD plugin to have more control over how mesh colliders are generated."), PropIntegrateWithConcaveCollider.boolValue);
        ...
        if(fracturedComponent.SaveMeshDataToAsset && (bComputeFracturing || bComputeColliders || bSaveMeshAssets) && (m_bProgressCancelled == false))
        {
            bProgressBarCreated = true;
