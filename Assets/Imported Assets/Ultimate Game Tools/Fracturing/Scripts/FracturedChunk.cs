...
public class FracturedChunk : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        /*PhysicsObject phys = gameObject.AddComponent<PhysicsObject>();
        
        phys.SetStartValue(10000f * RelativeVolume);*/
                ...
                if(collisionInfo.bCancelCollisionEvent == false)
                {
                    ...
                    foreach(FracturedChunk chunk in listBreaks)
                    {
                        ...
                        if(collisionInfo.bCancelCollisionEvent == false)
                        {
                            ...
                            if (myRigidbody == null) throw new InvalidOperationException(string.Format("Where has the rigidbody for {0} gone?", gameObject.name));

                            myRigidbody.AddExplosionForce(relativeSpeed * FracturedObjectSource.EventDetachExitForce, other.transform.position, 0.0f, FracturedObjectSource.EventDetachUpwardsModifier);
                        }
                    }
                }
            }
    ...
    public void DetachFromObject(bool bCheckStructureIntegrity = true)
    {
        if(IsDestructibleChunk() && IsDetachedChunk == false && GetComponent<Rigidbody>())
        {
            ...
            transform.parent = GameObject.Find("_ChunkManager").transform;
            ...
            // Here we are going to add the value of the chunk to the score
            AddScoreOnDamage asod = gameObject.GetComponent<AddScoreOnDamage>();

            LerpEmissiveColour lerp = GetComponent<LerpEmissiveColour>();
            if (lerp != null) lerp.Interpolate();
            ...
            if(DontDeleteAfterBroken == false && FracturedObjectSource != null)
            {
                ...
                // I'm going to change this line so that the time to death is related to the objects relative mass.
