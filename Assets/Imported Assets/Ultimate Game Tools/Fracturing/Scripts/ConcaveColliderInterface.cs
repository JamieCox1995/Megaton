...
namespace UltimateFracturing
{
    public static class ConcaveColliderInterface
    {
        ...
        private static bool ComputeHull(GameObject gameObject, bool isTrigger, FracturedObject.ECCAlgorithm eAlgorithm, int nMaxHulls, int nMaxHullVertices, int nLegacySteps, bool bVerbose, out int nTotalTrianglesOut)
        {
            ...
            IEnumerable<Mesh> decomposition;
            ConvexDecompositionParameters parameters = ConvexDecompositionParameters.Default;
            parameters.MaxHullCount = 4;//nMaxHulls;
            parameters.MaxVertices = nMaxHullVertices;
            //parameters.Depth = Mathf.CeilToInt(Mathf.Log(nMaxHulls, 2f));
            parameters.Resolution = 32768;
            parameters.NormalizeMesh = true;
            parameters.Verbose = bVerbose;

            nTotalTrianglesOut = 0;

            try
            {
                decomposition = MeshUtils.GetConvexDecomposition(theMesh.sharedMesh, parameters);
            }
            catch (Exception e)
            {
                if (bVerbose) Debug.LogException(e);

                return false;
            }

            int nHull = 0;
            foreach (Mesh hullMesh in decomposition)
            {
                GameObject goNewHull = gameObject;

                MeshCollider meshCollider = goNewHull.AddComponent<MeshCollider>();
                ...
                nHull++;
                nTotalTrianglesOut += hullMesh.triangles.Length / 3;
            }
#else
            DllInit(true);
