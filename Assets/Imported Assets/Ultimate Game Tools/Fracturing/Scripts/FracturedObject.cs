...
public class FracturedObject : MonoBehaviour
{
    ...
    public bool initializeSettingsForMegaton = true;

    public float buildingValue = 100000f;
    public float explosionForceThreshold = 50f;
    public float chunkVapourisationForce = 600f;
    public Color finalEmissionColor = Color.black;
    public float emissionLerpTime = 20f;
    ...
    public void SetSingleMeshVisibility(bool bEnabled)
    {
        ...
        foreach(FracturedChunk chunk in ListFracturedChunks)
        {
            if(chunk)
            {
                ...
                LerpEmissiveColour lerp = chunk.GetComponent<LerpEmissiveColour>();
        ...
        if(EventExplosionPrefabsArray.Length > 0 && EventExplosionPrefabsInstanceCount > 0 && ListFracturedChunks.Count > 0)
        {
            for(int i = 0; i < EventExplosionPrefabsInstanceCount; i++)
            {
                ...
                if(prefab != null)
                {
                    ...
                    {
                        FracturedChunk chunk = newGameObject.GetComponent<FracturedChunk>();

                        //DieTimer timer = newGameObject.AddComponent<DieTimer>();
                        //timer.SecondsToDie = UnityEngine.Random.Range(prefab.MinLifeTime, prefab.MaxLifeTime) * chunk.RelativeVolume;
                    }
                }
            }
        }
    ...
    public void Explode(Vector3 v3ExplosionPosition, float fExplosionForce, float fRadius, bool bPlayExplosionSound, bool bInstanceExplosionPrefabs, bool bAlsoExplodeFree, bool bCheckStructureIntegrityAfter)
    {
        ...
        foreach(FracturedChunk chunk in GetDestructibleChunksInRadius(v3ExplosionPosition, fRadius, bAlsoExplodeFree))
        {
            if(chunk)
            {
                // Why the fuck did this not check the force being applied to the object as it allows for the force to be 0 and a building crumbles
                float distance = Vector3.Distance(chunk.transform.position, v3ExplosionPosition);

                // scales the radius so that at fRadius the force is 1% of fExplosionForce
                float force = fExplosionForce / (distance * distance + 1);

                if (Mathf.Abs(force) >= explosionForceThreshold)
                {
                    AddScoreOnDamage asod = chunk.GetComponent<AddScoreOnDamage>();
                    // First of all, we are going to check to see if the object is vaporised
                    if (asod && force > 0f)
                    {
                        asod.CheckForVapourisation(force);
                    }

                    chunk.DetachFromObject(false);
                    chunk.GetComponent<Rigidbody>().AddExplosionForce(fExplosionForce, v3ExplosionPosition, fRadius, 0.0f, ForceMode.Impulse);
                    //chunk.GetComponent<AddScoreOnDamage>().DealDamage(force);     // We are moving this so that the score gets added when the chunk gets detached.
        ...
        foreach(FracturedChunk chunk in ListFracturedChunks)
        {
            if(chunk)
            {
                ...
                if(meshFilter)
                {
                    ...
                    // Checking to see if we are meant to be adding the megaton stuff
                    if (initializeSettingsForMegaton)
                    { 
                        AddScoreOnDamage physObject = chunk.gameObject.AddComponent<AddScoreOnDamage>();
                        physObject.SetObjectSettings(buildingValue * (fChunkVolume / fTotalVolume), explosionForceThreshold, chunkVapourisationForce);

                        ChunkDestroyer destroyer = chunk.gameObject.AddComponent<ChunkDestroyer>();
                        destroyer.deathTimer = UnityEngine.Random.Range(EventDetachedMinLifeTime, EventDetachedMaxLifeTime);

                        LerpEmissiveColour lec = chunk.gameObject.AddComponent<LerpEmissiveColour>();
                        lec._targetColor = finalEmissionColor;
                        lec.lerpTime = emissionLerpTime;
                    }
                }
            }
        }
    ...
    public void NotifyChunkDetach(FracturedChunk chunk)
    {
        ...
        {
            // Disable all triggers in them so that free chunks will collide also with static non-free chunks
            ...
            GameEventManager.instance.TriggerEvent(new ObjectDamagedEvent(this));
        }
    ...
    {
        // Lets iterate over all of the chunks
        for (int index = 0; index < ListFracturedChunks.Count; index++)
        {
            FracturedChunk chunk = ListFracturedChunks[index];

            // Get the rigidbody
            Rigidbody c_rigidbody = chunk.gameObject.GetComponent<Rigidbody>();
            // Update the mass of the chunk
            c_rigidbody.mass = chunk.RelativeVolume * TotalMass;

            ChunkDestroyer destroyer = chunk.gameObject.GetComponent<ChunkDestroyer>();

            if (!destroyer)
            {
                destroyer = chunk.gameObject.AddComponent<ChunkDestroyer>();

            }

            destroyer.deathTimer = UnityEngine.Random.Range(EventDetachedMinLifeTime * chunk.RelativeVolume, EventDetachedMaxLifeTime * chunk.RelativeVolume);

            if (initializeSettingsForMegaton)
            {
                // Get the AddScoreOnDamage
                AddScoreOnDamage asod = chunk.gameObject.GetComponent<AddScoreOnDamage>();
                asod.SetObjectSettings(buildingValue * chunk.RelativeVolume, explosionForceThreshold, chunkVapourisationForce * chunk.RelativeVolume);

                // Get the Lerp Emission Colour
                LerpEmissiveColour lec = chunk.gameObject.GetComponent<LerpEmissiveColour>();
                lec._targetColor = finalEmissionColor;
                lec.lerpTime = emissionLerpTime;
            }
        }
