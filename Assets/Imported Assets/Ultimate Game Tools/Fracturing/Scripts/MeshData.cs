...
namespace UltimateFracturing
{
    ...
    public class VertexData
    {
        ...
        public static VertexData[] BuildVertexDataArray(Mesh mesh, Matrix4x4 mtxLocalToWorld, bool bTransformVerticesToWorld)
        {
            ...
            for(int i = 0; i < mesh.vertexCount; i++)
            {
                ...
                else resultArray[i].bHasColor32 = false;
    ...
    public class MeshData
    {
...
#else
                        LookForClosedObject(connectivity, meshConnectivity, nSubMesh, alistFacesRemainingInOut[nSubMesh][0], alistIndices, listVertexData, alistFacesRemainingInOut, aLinearFaceIndexStart, alistIndicesOut, listVertexDataOut, dicVertexRemap, meshConnectivityOut);
#endif
                        // Build transform
        ...
        private static void LookForClosedObject(MeshFaceConnectivity connectivity,
                                                MeshDataConnectivity meshConnectivity,
                                                int nSubMeshStart,
                                                int nFaceSubMeshStart,
                                                List<int>[] alistIndicesIn,
                                                List<VertexData> listVertexDataIn,
                                                List<int>[] alistFacesRemainingInOut,
                                                int[] aLinearFaceIndexStart,
                                                List<int>[] alistIndicesOut,
                                                List<VertexData> listVertexDataOut,
                                                Dictionary<int, int> dicVertexRemap,
                                                MeshDataConnectivity meshConnectivityOut)
        {
            MeshFaceConnectivity.TriangleData initialTriangle = connectivity.listTriangles[aLinearFaceIndexStart[nSubMeshStart] + nFaceSubMeshStart];

            Queue<MeshFaceConnectivity.TriangleData> triangleQueue = new Queue<MeshFaceConnectivity.TriangleData>(new [] {initialTriangle});
            Queue<int> subMeshStartQueue = new Queue<int>(new [] {nSubMeshStart});
            Queue<int> faceSubMeshStartQueue = new Queue<int>(new []{nFaceSubMeshStart});

            while (triangleQueue.Count > 0)
            {
                MeshFaceConnectivity.TriangleData triangle = triangleQueue.Dequeue();
                nSubMeshStart = subMeshStartQueue.Dequeue();
                nFaceSubMeshStart = faceSubMeshStartQueue.Dequeue();
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    for (int nSide = 0; nSide < triangle.alistNeighborSubMeshes[i].Count; nSide++)
                    {
                        ...
                        if (triangleAdjacent != null)
                        {
                            if (triangleAdjacent.bVisited != true)
                            {
                                triangleQueue.Enqueue(triangleAdjacent);
                                subMeshStartQueue.Enqueue(triangleAdjacent.nSubMesh);
                                faceSubMeshStartQueue.Enqueue(triangleAdjacent.nTriangle);
                            }
                        }
                    }
                }
