    ...
    public static partial class Fracturer
    {
        ...
        public static bool FractureToChunks(FracturedObject fracturedComponent, bool bPositionOnSourceAndHideOriginal, out List<GameObject> listGameObjectsOut, ProgressDelegate progress = null)
        {
            ...
            if(gameObjectIn)
            {
                ...
                uvRemap = fracturedComponent.SourceObject.GetComponent<RuntimeUVRemap>();
            }
            ...
            if (bFracturingOK && uvRemap != null)
            {
                for (int i = 0; i < listGameObjectsOut.Count; i++)
                {
                    RuntimeUVRemap chunkUvRemap = listGameObjectsOut[i].AddComponent<RuntimeUVRemap>();
                    chunkUvRemap.Mode = uvRemap.Mode;
                    chunkUvRemap.TextureRects = uvRemap.TextureRects;

                    List<Material> materials = new List<Material>(uvRemap.Materials);
                    materials.Add(fracturedComponent.SplitMaterial);

                    chunkUvRemap.Materials = materials;
                }
            }
            ...
            if(bFracturingOK && Fracturer.IsFracturingCancelled() == false)
            {
                ...
                for(int i = 0; i < aComponents.Length; i++)
                {
                    ...
                    if(c.GetType() != typeof(Transform) && c.GetType() != typeof(MeshRenderer) && c.GetType() != typeof(MeshFilter) && c.GetType() != typeof(RuntimeUVRemap))
                    {
...
*/
        ...
        private static Mesh CopyMesh(MeshFilter meshfIn)
        {
            ...
            newMesh.indexFormat = meshfIn.sharedMesh.indexFormat;
                ...
                {                     
                    ...
                    else
                    {
                        ...
                        if(meshFilter != null && meshFilter.sharedMesh != null && meshFilter.sharedMesh.vertexCount >= 3)
                        {
                            ...
                            newCollider.isTrigger = fracturedComponent.ChunkColliderType == FracturedObject.ColliderType.Trigger;
                        }
                    }
                }
