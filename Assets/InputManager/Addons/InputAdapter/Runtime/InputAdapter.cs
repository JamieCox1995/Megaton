...
namespace TeamUtility.IO
{
	...
	public sealed class InputAdapter : MonoBehaviour
	{
		...
		private void SetInputDevice(InputDevice inpuDevice)
		{
			...
			{
                // Locking and hiding the mouse if a joy pad is detected.
				Cursor.visible = false;
                Cursor.lockState = CursorLockMode.Locked;

                // Getting canvases and disabling their raycaster so the mouse cannot interfere with it
                foreach (Canvas can in FindObjectsOfType<Canvas>())
                {
                    can.GetComponent<UnityEngine.UI.GraphicRaycaster>().enabled = false;
                }
