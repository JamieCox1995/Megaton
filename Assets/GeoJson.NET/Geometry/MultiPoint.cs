...
namespace GeoJSON.Net.Geometry
{
    ...
    public class MultiPoint : GeoJSONObject, IGeometryObject, IEqualityComparer<MultiPoint>, IEquatable<MultiPoint>
    {
        ...
        {
            Coordinates = new ReadOnlyCollection<Point>((coordinates != null) ? coordinates.ToArray() : (new Point[0]));
        }
        
        [JsonConstructor]
        public MultiPoint(IEnumerable<IEnumerable<double>> coordinates)
        : this(coordinates != null ? coordinates.Select(position => new Point(position.ToPosition())) : null)
        {
            if (this == null)
        ...
        public ReadOnlyCollection<Point> Coordinates { get; private set; }
